...
def image_to_matrix(image_root, dest, size):
	...
	if len(train_data) != 0:
		train_data, train_label = shuffle(train_data, train_label)
		np.save(dest + "train_data1.npy", np.array(train_data))  # model root to save image models(image)
		np.save(dest + "train_label1.npy", np.array(train_label))  # model root to save image models(label)
